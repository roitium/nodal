...
export default function Home() {
  ...
  const fetchMemos = async () => {
    try {
      // Pass token if available to see private memos
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const data = await apiFetch<any>("/api/v1/memos/timeline?limit=10", {
        token,
      });
      setMemos(data.data);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. Get Upload URL
      const { uploadUrl, path } = await apiFetch<any>(
      ...
      // 3. Record Upload
      const resource = await apiFetch<any[]>(
        ...
        <Card className="mb-8">
            <CardContent className="pt-6">
                        ...
                        {resources.map(res => (
                            <div key={res.id} className="relative group">
                                {res.type.startsWith('image/') ? (
                                    <img src={res.externalLink} alt="preview" className="h-20 w-20 object-cover rounded-md border" />
                                ) : (
                ...
                <div className="flex justify-between items-center">
                    ...
                    <Button onClick={handlePost} disabled={isPosting || isUploading || (!content.trim() && resources.length === 0)}>
